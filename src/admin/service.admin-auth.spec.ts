...
const mockPrisma = {
  ...
  adminToken: { findFirst: jest.fn(), create: jest.fn(), update: jest.fn(), updateMany: jest.fn() },
...
describe('AdminAuthService', () => {
  ...
  describe('login', () => {
    ...
    it('throws generic error when password is null (pending invite)', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue({ ...ACTIVE_ADMIN, passwordHash: null });
      await expect(service.login({ email: ACTIVE_ADMIN.email, password: 'pass' })).rejects.toThrow(UnauthorizedException);
    });

    it('throws generic error when admin is not ACTIVE', async () => {
      const hash = await argon2.hash('correct');
      mockPrisma.admin.findUnique.mockResolvedValue({ ...ACTIVE_ADMIN, passwordHash: hash, status: { name: 'SUSPENDED' } });
      await expect(service.login({ email: ACTIVE_ADMIN.email, password: 'correct' })).rejects.toThrow(UnauthorizedException);
    });

    it('throws on wrong password and logs LOGIN_FAILED', async () => {
      const hash = await argon2.hash('correct');
      mockPrisma.admin.findUnique.mockResolvedValue({ ...ACTIVE_ADMIN, passwordHash: hash });
      await expect(service.login({ email: ACTIVE_ADMIN.email, password: 'wrong' })).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('sends OTP email on valid credentials', async () => {
      ...
      const result = await service.login({ email: ACTIVE_ADMIN.email, password: 'correct' });
  ...
  describe('verifyOtp', () => {
    ...
    it('throws when account locked (too many attempts)', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue({ ...ACTIVE_ADMIN, invalidOtpCount: 5 });
      await expect(service.verifyOtp({ email: ACTIVE_ADMIN.email, otp: '123456' })).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('throws and increments count on invalid OTP', async () => {
      ...
      mockPrisma.adminToken.findFirst.mockResolvedValue({ id: 'tok1', token: hash });
      mockPrisma.admin.update.mockResolvedValue({});

      await expect(service.verifyOtp({ email: ACTIVE_ADMIN.email, otp: '999999' })).rejects.toThrow(UnauthorizedException);
      expect(mockPrisma.admin.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: { invalidOtpCount: { increment: 1 } } }),
      );
    });
    ...
    it('returns tokens on valid OTP', async () => {
      ...
      const result = await service.verifyOtp({ email: ACTIVE_ADMIN.email, otp: '123456' });
  ...
  describe('acceptInvite', () => {
    it('throws on invalid token', async () => {
      ...
      await expect(service.acceptInvite({ token: 'bad', password: 'pass123' })).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws if admin is not PENDING', async () => {
      ...
      await expect(service.acceptInvite({ token: 'goodtoken', password: 'pass123' })).rejects.toThrow(BadRequestException);
    });
    ...
    it('activates account on valid invite', async () => {
      ...
      const result = await service.acceptInvite({ token: 'goodtoken', password: 'NewPass123!' });
