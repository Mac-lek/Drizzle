...
const mockPrisma = {
  admin: { findUnique: jest.fn(), create: jest.fn(), findMany: jest.fn(), update: jest.fn() },
...
describe('AdminService', () => {
  ...
  describe('invite', () => {
    it('throws ForbiddenException when inviting SADM role', async () => {
      await expect(service.invite('adm_1', { email: 'new@test.com', roleCode: 'SADM' })).rejects.toThrow(ForbiddenException);
    });

    it('throws BadRequestException for unknown role', async () => {
      mockPrisma.adminRole.findUnique.mockResolvedValue(null);
      await expect(service.invite('adm_1', { email: 'new@test.com', roleCode: 'GHOST' })).rejects.toThrow(BadRequestException);
    });

    it('throws ConflictException when email already exists', async () => {
      mockPrisma.adminRole.findUnique.mockResolvedValue({ code: 'SUPPORT', name: 'Customer Support' });
      mockPrisma.admin.findUnique.mockResolvedValue({ id: 'adm_existing' });
      await expect(service.invite('adm_1', { email: 'existing@test.com', roleCode: 'SUPPORT' })).rejects.toThrow(ConflictException);
    });

    it('creates admin, token, and sends email on valid invite', async () => {
      mockPrisma.adminRole.findUnique.mockResolvedValue({ code: 'SUPPORT', name: 'Customer Support' });
      mockPrisma.admin.findUnique.mockResolvedValue(null);
      mockPrisma.adminStatus.findUniqueOrThrow.mockResolvedValue(PENDING_STATUS);
      mockPrisma.admin.create.mockResolvedValue({ id: 'adm_new' });
      mockPrisma.adminToken.create.mockResolvedValue({});

      const result = await service.invite('adm_1', { email: 'new@test.com', roleCode: 'SUPPORT' });
  ...
  describe('updatePermissions', () => {
    it('throws NotFoundException when target does not exist', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue(null);
      await expect(service.updatePermissions('adm_1', 'adm_x', { permissions: [] })).rejects.toThrow(NotFoundException);
    });

    it('throws ForbiddenException when target is SADM', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue({ id: 'adm_sadm', roleCode: 'SADM' });
      await expect(service.updatePermissions('adm_1', 'adm_sadm', { permissions: [] })).rejects.toThrow(ForbiddenException);
    });

    it('upserts permissions in transaction', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue({ id: 'adm_2', roleCode: 'SUPPORT' });
  ...
  describe('updateStatus', () => {
    it('throws NotFoundException when target does not exist', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue(null);
      await expect(service.updateStatus('adm_1', 'adm_x', { status: 'ACTIVE' })).rejects.toThrow(NotFoundException);
    });

    it('throws ForbiddenException when target is SADM', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue({ id: 'adm_sadm', roleCode: 'SADM', role: {} });
      await expect(service.updateStatus('adm_1', 'adm_sadm', { status: 'SUSPENDED' })).rejects.toThrow(ForbiddenException);
    });

    it('throws ForbiddenException when changing own status', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue({ id: 'adm_1', roleCode: 'SUPPORT', role: {} });
      await expect(service.updateStatus('adm_1', 'adm_1', { status: 'SUSPENDED' })).rejects.toThrow(ForbiddenException);
    });

    it('throws BadRequestException for unknown status', async () => {
      mockPrisma.admin.findUnique.mockResolvedValue({ id: 'adm_2', roleCode: 'SUPPORT', role: {} });
      mockPrisma.adminStatus.findUnique.mockResolvedValue(null);
      await expect(service.updateStatus('adm_1', 'adm_2', { status: 'GHOST' })).rejects.toThrow(BadRequestException);
    });
