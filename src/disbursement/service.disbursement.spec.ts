...
describe('DisbursementService', () => {
  ...
  describe('findByUser', () => {
    it('returns disbursements ordered by createdAt desc', async () => {
      (prisma.disbursement.findMany as jest.Mock).mockResolvedValue([mockDisbursement]);

      const result = await service.findByUser('user-1');

      expect(prisma.disbursement.findMany).toHaveBeenCalledWith({
        where: { userId: 'user-1' },
        select: expect.objectContaining({ id: true, vaultId: true, dripNumber: true }),
  ...
  describe('findById', () => {
    it('returns the disbursement when found and owned by user', async () => {
      (prisma.disbursement.findFirst as jest.Mock).mockResolvedValue(mockDisbursement);
    ...
    it('throws NotFoundException when disbursement not found', async () => {
      ...
      await expect(service.findById('dsb_notexist', 'user-1')).rejects.toThrow(NotFoundException);
    });

    it('throws NotFoundException when disbursement belongs to another user', async () => {
      (prisma.disbursement.findFirst as jest.Mock).mockResolvedValue(null);
      await expect(service.findById('dsb_abc123', 'user-other')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('findDueVaults', () => {
    it('returns only vaults whose next drip time has passed', async () => {
      ...
      (prisma.vaultStatus.findUniqueOrThrow as jest.Mock).mockResolvedValue({ id: 1 });
