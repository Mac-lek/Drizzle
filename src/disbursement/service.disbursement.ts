...
export class DisbursementService {
  ...
  > {
    const now = new Date();
    const activeStatus = await this.prisma.vaultStatus.findUniqueOrThrow({
      where: { name: "ACTIVE" },
    });
    ...
    return vaults.filter((v) => {
      ...
      return (
        this.computeNextDripAt(v.startsAt, v.tranchesSent, v.frequency.name) <=
        now
      );
    });
  }
  ...
  async processDrip(vaultId: string): Promise<void> {
    ...
    const now = new Date();
    const nextDripAt = this.computeNextDripAt(
      vault.startsAt,
      vault.tranchesSent,
      vault.frequency.name,
    );
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        ...
        const [
          processingStatus,
          completedDisbStatus,
          completedVaultStatus,
          debitDir,
          creditDir,
          vaultType,
          walletType,
        ] = await Promise.all([
          tx.disbursementStatus.findUniqueOrThrow({
        ...
        await tx.vault.update({
          ...
          data: {
            ...
            ...(newTranchesSent === vault.totalTranches && {
              statusId: completedVaultStatus.id,
            }),
          },
        });
      ...
      this.logger.error(
        { vaultId, dripNumber, error },
        "Drip processing failed",
      );

      const failedStatus =
        await this.prisma.disbursementStatus.findUniqueOrThrow({
          where: { name: "FAILED" },
        });
