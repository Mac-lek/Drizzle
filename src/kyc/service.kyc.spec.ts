...
describe('KycService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: DojahProvider, useValue: { verifyBvn: jest.fn() } },
        {
          ...
          useValue: { createVerificationLink: jest.fn(), verifyWebhookSignature: jest.fn() },
        },
  ...
  describe('getStatus', () => {
    it('returns kycStatus and bvnVerified', async () => {
      ...
      expect(await service.getStatus('usr_1')).toEqual({ kycStatus: 'TIER_1_VERIFIED', bvnVerified: true });
    });
  });
  ...
  describe('submitTier1', () => {
    it('verifies BVN, encrypts it, and sets TIER_1_VERIFIED', async () => {
      ...
      (dojah.verifyBvn as jest.Mock).mockResolvedValue({ firstName: 'JOHN', lastName: 'DOE', dateOfBirth: '', phoneNumber: '' });
    ...
    it('sets FAILED and re-throws when Dojah fails', async () => {
      ...
      expect((prisma.user.update as jest.Mock).mock.calls[0][0].data.kycStatusId).toBe(6);
    });
  ...
  describe('initiateTier2', () => {
    it('creates a Smile link and sets TIER_2_PENDING', async () => {
      (prisma.user.findUniqueOrThrow as jest.Mock).mockResolvedValue(mockUser('TIER_1_VERIFIED'));
      (prisma.kycStatus.findUniqueOrThrow as jest.Mock).mockResolvedValue({ id: 4 });
      (smile.createVerificationLink as jest.Mock).mockResolvedValue('https://links.usesmileid.com/abc');
      (prisma.user.update as jest.Mock).mockResolvedValue({});
    ...
    it('throws ConflictException if Tier 1 not completed', async () => {
      (prisma.user.findUniqueOrThrow as jest.Mock).mockResolvedValue(mockUser('NONE'));
      await expect(service.initiateTier2('usr_1')).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('handleSmileCallback', () => {
    ...
    it('sets TIER_2_VERIFIED on ResultCode 1012', async () => {
      ...
      (prisma.kycStatus.findUniqueOrThrow as jest.Mock).mockResolvedValue({ id: 5 });
      (prisma.user.update as jest.Mock).mockResolvedValue({});
      (prisma.webhookEvent.update as jest.Mock).mockResolvedValue({});

      await service.handleSmileCallback(baseBody);

      expect(prisma.kycStatus.findUniqueOrThrow).toHaveBeenCalledWith({ where: { name: 'TIER_2_VERIFIED' } });
    });
    ...
    it('sets FAILED on non-1012 ResultCode', async () => {
      ...
      (prisma.webhookEvent.findUnique as jest.Mock).mockResolvedValue(null);
      (prisma.webhookEvent.upsert as jest.Mock).mockResolvedValue({});
      (prisma.kycStatus.findUniqueOrThrow as jest.Mock).mockResolvedValue({ id: 6 });
      (prisma.user.update as jest.Mock).mockResolvedValue({});
      (prisma.webhookEvent.update as jest.Mock).mockResolvedValue({});

      await service.handleSmileCallback({ ...baseBody, ResultCode: '1020' });

      expect(prisma.kycStatus.findUniqueOrThrow).toHaveBeenCalledWith({ where: { name: 'FAILED' } });
    });

    it('skips processing if event already processed', async () => {
      (smile.verifyWebhookSignature as jest.Mock).mockReturnValue(undefined);
      (prisma.webhookEvent.findUnique as jest.Mock).mockResolvedValue({ processed: true });
    ...
    it('throws UnauthorizedException on bad signature', async () => {
      ...
      await expect(service.handleSmileCallback(baseBody)).rejects.toThrow(UnauthorizedException);
      expect(prisma.user.update).not.toHaveBeenCalled();
    });
  });
});
