...
export class VaultService {
  ...
  async create(
    ...
    const [frequency, activeStatus] = await Promise.all([
      this.prisma.dripFrequency.findUniqueOrThrow({
        where: { name: dto.frequency },
      }),
  ...
  async breakVault(
    ...
    const penaltyKobo =
      (remainingKobo * BigInt(Math.round(BREAK_PENALTY_RATE * 1000))) /
      BigInt(1000);
    ...
    await this.prisma.$transaction(async (tx) => {
      const [debitId, creditId, vaultTypeId, walletTypeId, penaltyTypeId] =
        await Promise.all([
          this.resolveDirectionId(tx, "DEBIT"),
          this.resolveDirectionId(tx, "CREDIT"),
          this.resolveAccountTypeId(tx, "VAULT"),
