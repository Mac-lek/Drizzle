...
describe("AuthService", () => {
  ...
  describe("verifyOtp", () => {
    ...
    it("throws UnauthorizedException when phone is not found", async () => {
      ...
      await expect(
        service.verifyOtp({ identifier: "08012345678", otp: "123456" }),
  ...
  describe("login", () => {
    beforeEach(() => {
      (prisma.userStatus.findUnique as jest.Mock).mockResolvedValue({
        id: 1,
        name: "ACTIVE",
      });
      (prisma.tokenType.findUniqueOrThrow as jest.Mock).mockResolvedValue({
    ...
    it("throws UnauthorizedException for unknown identifier", async () => {
      ...
      await expect(
        service.login({ identifier: "08099999999", pin: "1234" }),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it("throws UnauthorizedException for wrong PIN", async () => {
      ...
      await expect(
        service.login({ identifier: "08012345678", pin: "0000" }),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it("throws ForbiddenException when user is not ACTIVE", async () => {
      ...
      await expect(
        service.login({ identifier: "08012345678", pin: "1234" }),
