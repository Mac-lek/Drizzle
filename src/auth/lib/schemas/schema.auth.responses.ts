...
export class OtpVerifiedResponse {
  ...
  @ApiProperty({
    description: "Short-lived JWT — use only to call POST /auth/set-pin",
  })
  accessToken: string;
}
...
export class TokenPairResponse {
  ...
  @ApiProperty({
    description: "Opaque refresh token (30 days) — store securely",
  })
  refreshToken: string;
}
...
export class RefreshResponse {
  ...
  @ApiProperty({
    description: "New refresh token — old token is immediately invalidated",
  })
  refreshToken: string;
}
